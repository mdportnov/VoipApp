...
struct CallerScreen: View {
    ...
    var receiverID: String = "1234"
    var hasActivateCall: Bool = true
                    ...
                    HStack {
                        ...
                        Button(action: {
                            if !state.isNumPadVisible {
                                ...
                                tryToCall(sipName: state.inputLine)
                            }
                        }) {
    ...
    func startCall(to receiverID: String) {
//        let uuid = UUID()
//        self.callID = uuid

        callManager.startCall(with: callID, receiverID: receiverID) { error in
            if let error = error {
